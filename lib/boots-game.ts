...
/**
 * Updates the top 10 scores for a user's boots game
 * Note: For boots, lower seconds = better score, so we track the 10 lowest scores
 * If the seconds qualify (is in top 10 lowest), inserts it and removes any scores above the 10th lowest
 */
export async function updateTopScores(userId: string, seconds: number): Promise<void> {
  ...
  try {
    ...
    const qualifies = hasLessThan10 || (tenthLowestScore !== null && seconds <= tenthLowestScore);
...
/**
 * Gets the reward rarity based on seconds (time)
 * Queries boots_rewards_lookup to find the lowest max_score >= seconds
 * (Since lower seconds is better, we want the lowest max_score that qualifies)
 */
export async function getRewardRarity(seconds: number): Promise<string | null> {
  ...
  try {
    const { data, error } = await supabase
      .from("boots_rewards_lookup")
      .select("rarity")
      .gte("max_score", seconds)
      .order("max_score", { ascending: true })
      .limit(1)
      .single();

    if (error) {
      // If no rows found (PGRST116), that's okay - seconds don't qualify
...
/**
 * Gets a random boots by rarity
 * Queries boots_lookup and randomly selects one boots
 */
export async function getRandomBootsByRarity(
  ...
  try {
    ...
    if (error) {
      console.error("Error fetching boots by rarity:", error);
      return null;
    }
...
/**
 * Adds boots to the user's inventory
 * Inserts into user_inventory table with gear_type "Boots" and is_equipped false
 */
