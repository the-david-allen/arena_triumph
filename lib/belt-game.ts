...
/**
 * Updates the play count for a user's belt game
...
/**
 * Updates the top 10 scores for a user's belt game
 * Note: For belt, lower seconds = better score, so we track the 10 lowest scores
 ...
 * Queries belt_rewards_lookup to find the lowest max_score >= seconds
...
export async function getRandomBeltByRarity(rarity: string): Promise<{ id: string; name: string; image_url: string | null } | null> {
  ...
  try {
    ...
    if (error) {
      console.error("Error fetching belts by rarity:", error);
      return null;
    }
...
/**
 * Adds belt to the user's inventory
 * Inserts into user_inventory table with gear_type "Belt" and is_equipped false
 */
